...
def calculate_pipeline_status(event: dict, current_steps: list):
    ...
    try:
        ...
        with engine.begin() as connection:
            ...
            connection.execute(
                ...
                """),
                {"pipeline_id": pipeline_id, "event_type": event_type, "payload": json.dumps(event)}
            )
            if event_type == "STEP_STARTED":
                ...
                connection.execute(
                    ...
                    WHERE pipeline_id = :pipeline_id AND step_name = :step_name
                    """),
                    {"status": step_status, "pipeline_id": pipeline_id, "step_name": event["step_name"]}
                )
